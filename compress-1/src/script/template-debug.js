...
(function () {
    ...
    var template = function (filename, content) {
        return typeof content === 'string' ? compile(content, { filename: filename }) : renderFile(filename, content);
    };
    ...
    var emptyContinue = function (code) {
        var newCode = [];
        for (var key in code) {
            var value = code[key];
            if (value === '') {
    ...
    defaults.parser = function (code, options) {
        ...
        // split.shift();
        code = code.replace(/^\s/, '');
        var splitEmpty = code.split(' ');
        var split = emptyContinue(splitEmpty);
        var key = split.shift();
        var args = split.join(' ');
        //console.log(key, code, split);
        switch (key) {
            case '~': // 自定义参数忽略执行
                code = '' + args + ';';
                break;
